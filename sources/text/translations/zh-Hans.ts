...
export const zhHans: TranslationStructure = {
    ...
    connect: {
        ...
        serverConnectionFailed: ({ server }: { server: string }) => `无法连接到服务器：${server}`,
        authenticationFailed: '认证失败。服务器拒绝了您的凭据。',
    ...
    newSession: {
        ...
        title: '启动新会话',
        noMachinesFound: '未找到此账户的设备。',
        noMachinesFoundHelp: '请确保 CLI 守护进程正在您的计算机上以相同账户运行。',
        noMachinesAccountInfo: ({ accountId }: { accountId: string }) => `账户：${accountId}`,
        noMachinesTroubleshoot: '故障排除提示：',
        noMachinesTip1: '• 在您的计算机上运行 `happy daemon start`',
        noMachinesTip2: '• 验证 CLI 已通过身份验证：`happy auth status`',
        noMachinesTip3: '• 确保 CLI 和应用使用相同的服务器',
    ...
    settingsAccount: {
        ...
        name: '姓名',
        username: '用户名',
        usernameSubtitle: '点击编辑',
        usernameNotSet: '未设置',
        editUsername: '编辑用户名',
        editUsernameDescription: '选择一个唯一的用户名（3-30个字符，字母、数字、下划线、连字符）',
